...
def startup():
    ...
    for attempt in range(max_retries):
        try:
            print(f"\n[INFO] Intento de conexi贸n a DB: {attempt + 1}/{max_retries}...")
            models.Base.metadata.create_all(bind=database.engine)
            print("[EXITO] Conexi贸n establecida y tablas verificadas.\n")
            break
        except OperationalError as e:
            print(f"[WARN] La base de datos a煤n no responde. Reintentando en {wait_seconds}s...")
            time.sleep(wait_seconds)
    else:
        print("\n[CRITICO] No se pudo conectar a la DB despu茅s de varios intentos.\n")


@app.get("/")
def read_root():
    return {"mensaje": "API de Gesti贸n Acad茅mica Operativa", "docs": "/docs"}
...
def crear_estudiante(student: schemas.StudentCreate, db: Session = Depends(database.get_db)):
    ...
    if db_student:
        raise HTTPException(status_code=400, detail="El c贸digo de matr铆cula ya existe.")
